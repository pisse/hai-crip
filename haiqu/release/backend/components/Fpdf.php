...
define('FPDF_FONTPATH',dirname(__FILE__).'/font/'); //定义font文件夹所在路径
...
function Write($h, $txt, $link='')
{
    ...
    $cw = &$this->CurrentFont['cw'];
    $w = $this->w-$this->rMargin-$this->x;
    $this->FontSize = empty($this->FontSize) ? 2 : $this->FontSize;
    $wmax = ($w-2*$this->cMargin)*1000/$this->FontSize;
    $s = str_replace("\r",'',$txt);
    $nb = strlen($s);
    $sep = -1;
 ...
 *******************************************************************************/
    ...
    function CN_MultiCell($w, $h, $txt, $border=0, $align='L', $fill=0)
    {
        if($this->CurrentFont['type']=='Type0')
