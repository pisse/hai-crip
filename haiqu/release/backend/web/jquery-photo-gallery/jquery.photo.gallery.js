	...
	photoGallery: function(options) {
		...
		function mouseWheelScroll(e){
	  	...
	  	function toggleImage(){
	    	imageWidth = o.imgs.imgWidth;
       		imageHeight = o.imgs.imgHeight;
			...
			$image = $(".image[index='"+o.activeIndex+"']", $gallery).addClass("active").css({
	  		...
	  		$thumbImg = $thumbnails.find("img").attr("src", o.imgs.url);	
...
$.extend({
	...
	openPhotoGallery : function(obj){
		...
		if(imgHeight > wH){
			imgWidth = wH/(imgHeight/imgWidth);
			imgHeight = wH;
		}

		//imgHeight = Math.min(wH,imgHeight);
		//imgWidth = Math.min(wW,imgWidth);
		...
		$("<iframe></iframe").appendTo("body")
			...
			.css({
				position : "absolute",
				left : (document.body.clientWidth - $(window).width()) /2,
	...
	initGallery : function(){
		...
		$(".closeWin").click(function(){
			...
			_parent.document.body.style.overlfow='auto';
			_parent.document.documentElement.style.overflow = 'auto';	
			$(_jg).remove();
