...
// Version: 1.1.1
// Funcnames:
// == not_blank int uint float decimal uint_range length eq
...
class ValidatorHelper {
    ...
    function getValidData($passFields = array()) {
        $attribute = array ();
        foreach ( $this->valid as $attrName => $val ) {
            if (in_array ( $attrName, $passFields ) || $val === NULL) {
                continue;
            }
            $attribute [$attrName] = $val;
        }
        return $attribute;
    }
    // ================检验变量的方法（变量可以是数组）================
    ...
    // 'name' => array('int'),
    // ));
    private function int($value, $func_param) {
        if (is_string ( $value ) && preg_match ( "/^-?\d*$/", $value ) || is_int ( $value ) || empty ( $value )) {
            return array (
                    TRUE 
            );
        }
    ...
    // ));
    private function uint_range($value, $func_param) {
